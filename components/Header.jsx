...
export default function Header({ title, description }) {
  ...
  return (
    <header className="py-3 sticky top-0 z-10 backdrop-blur-sm">
      ...
      <div className="flex items-center justify-between container mx-auto px-5 md:px-0">
        ...
        <div className="flex items-center space-x-10 text-gray-500">
          ...
          </button>
          {
            ...
            <button onClick={() =>setShowNotifications(true)} className="bg-gray-100 p-2 rounded-full flex">
      ...
      {showNotifications && <div onClick={() =>setShowNotifications(false)} className="absolute flex top-0 left-0  w-screen h-screen z-10 bg-black/20 ">

        <div className="flex-1 relative container mx-auto">
          <div onClick={e=>{e.stopPropagation();}} className="bg-white h-[100%] md:w-[25vw] md:h-auto md:min-h-32  flex  flex-col rounded-md absolute top-20 md:right-40">
            ...
            <div className="border-b  px-3 p-3 flex items-center justify-between">
              ...
              <button onClick={() =>setShowNotifications(false)} className="bg-gray-100 rounded-full p-1">
                <HiX size={18} />
              </button>
            </div>
            <div className="flex flex-col flex-1 p-5 space-y-5">
              <div onClick={()=>router.push('/orders/4')} className=" cursor-pointer flex flex-row  space-x-3 border-b border-gray-100 pb-5">
                ...
                <div className="flex flex-col space-y-1">
                  <h3 className="text-sm font-medium text-gray-600">Order #778748</h3>
                  <p className="text-sm font-light text-gray-600">Your order #778748 has been is beign prepared. and will be ready for pickup in 10 mins time.</p>
                  <p className="text-xs text-yellow-600 font-medium">Just now</p>
                </div>
              </div>
              <div onClick={()=>router.push('/orders/4')} className=" cursor-pointer flex flex-row  space-x-3  pb-5">
                <div className="h-10 shrink-0 w-10 flex items-center justify-center border rounded-full">
                  <GiMoneyStack color="green" />
                </div>
                <div className="flex flex-col space-y-1">
                  <h3 className="text-sm font-medium text-gray-600">Order #778748</h3>
                  <p className="text-sm font-light text-gray-600">Your order #778748 has been created successfully and awaiting confirmation.</p>
                  <p className="text-xs text-yellow-600 font-medium">Just now</p>
                </div>
              </div>
